...
const ManageEmployees = ({ token, onAddEmployeeClick }) => {
  ...
  const [formData, setFormData] = useState({ name: '', email: '', address: '', salary: '' ,phoneNumber:''});
  ...
  const handleEditClick = (emp) => {
    ...
    setFormData({ name: emp.name, email: emp.email, address: emp.address || '', salary: emp.salary || '' ,phoneNumber:emp.phoneNumber || ''});
  };
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ name: '', email: '', address: '', salary: '',phoneNumber:''});
    } catch (err) {
